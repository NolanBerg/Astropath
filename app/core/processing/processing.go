...
func (wp *WorkflowProcessor) ProcessImage() error {
	wp.eventManager.Emit("frame:start", wp.Batch.Files[wp.Processed])
	if wp.Processed >= uint(len(wp.Batch.Files)) {
		return fmt.Errorf("all files already processed")
	}

	incomingPath := wp.Batch.Files[wp.Processed]
	...
	status := ProcessStatus{
		...
		Total:       len(wp.Batch.Files),
		PreviewPath: outputFilePath,
	}

	fmt.Printf("Processed %v (%d/%d)\n", incomingPath, wp.Processed, len(wp.Batch.Files))
