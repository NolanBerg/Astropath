...
function createAppSettingsStore() {
    ...
    /** 
     * Load settings from disk. Makes call to go backend and then loads the value into the store. Will update the UI & all
     components which use the store.
    */
...
function createBatchStore() {
    const storeValue = writable<ImageBatch>({ FilePaths: [], Bounds: null });
