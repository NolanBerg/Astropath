...
(function (global, factory) {
  ...
  function detectWebfontConflicts() {
    ...
    function runDiag(scriptOrLinkTag, md5) {
      ...
      const parentOrigin = WINDOW.location.origin === "file://"
        ? "*"
        : WINDOW.location.origin;
      diagScript.innerText = "(".concat(diagScriptFun.toString(), ")('").concat(
        nodeUnderTestId,
        "', '",
      ).concat(testIconId || "foo", "', '").concat(md5, "', '").concat(
        parentOrigin,
        "');",
      );
  ...
  function detectSvgConflicts(currentScript) {
    const scripts = Array.from(DOCUMENT.scripts).filter((t) =>
      !t.hasAttribute(detectionIgnoreAttr) && t !== currentScript
    );
    const scriptsByMD5 = {};
  ...
  function conflictDetection() {
    let report$$1 = arguments.length > 0 && arguments[0] !== undefined
      ? arguments[0]
      : () => {};
    const nodesTested = {
      conflict: {},
    ...
    const testCount = Object.keys(scriptsToTest).length +
      Object.keys(cssToTest).length;
    ...
    const masterTimeout = WINDOW.FontAwesomeDetection.timeout +
    ...
    if (testCount === 0) {
      ...
      pollUntil({
        ...
        fn: () => {
          return Object.keys(nodesTested.conflict).length +
              Object.keys(nodesTested.noConflict).length >= testCount;
        },
      }).then(() => {
        console.info("DONE!");
        setDoneResults({
