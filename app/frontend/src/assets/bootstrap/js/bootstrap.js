...
(function (global, factory) {
  ...
  const toType = (object) => {
    ...
    return Object.prototype.toString.call(object).match(/\s([a-z]+)/i)[1]
      .toLowerCase();
  };
  /**
   * Public Util API
  ...
  class Alert extends BaseComponent {
    ...
    static jQueryInterface(config) {
      return this.each(function () {
        ...
        if (
          data[config] === undefined || config.startsWith("_") ||
          config === "constructor"
        ) {
  ...
  class Collapse extends BaseComponent {
    ...
    show() {
      ...
      if (this._config.parent) {
        activeChildren = this._getFirstLevelChildren(SELECTOR_ACTIVES).filter(
  ...
  class Dropdown extends BaseComponent {
    ...
    _getConfig(config) {
      ...
      if (
        typeof config.reference === "object" && !isElement(config.reference) &&
    ...
    _getPlacement() {
      ...
      const isEnd =
        getComputedStyle(this._menu).getPropertyValue("--bs-position")
          .trim() === "end";
  ...
  class FocusTrap extends Config {
    ...
    _handleFocusin(event) {
      ...
      if (
        event.target === document || event.target === trapElement ||
    ...
    _addEventListeners() {
      ...
      EventHandler.on(this._element, EVENT_MOUSEDOWN_DISMISS, (event) => {
        ...
        EventHandler.one(this._element, EVENT_CLICK_DISMISS, (event2) => {
          if (
            this._element !== event.target || this._element !== event2.target
          ) {
            return;
          }
  ...
  EventHandler.on(window, EVENT_RESIZE, () => {
    for (
      const element of SelectorEngine.find(
  ...
  class Tooltip extends BaseComponent {
    ...
    show() {
      ...
      const isInTheDom =
        (shadowRoot || this._element.ownerDocument.documentElement).contains(
          this._element,
        );
  ...
  class ScrollSpy extends BaseComponent {
    ...
    _configAfterMerge(config) {
      ...
      if (typeof config.threshold === "string") {
        config.threshold = config.threshold.split(",").map((value) =>
      ...
      EventHandler.on(
        ...
        (event) => {
          ...
          if (observableSection) {
            ...
            const height = observableSection.offsetTop -
    ...
    _activateParents(target) {
      ...
      for (
        const listGroup of SelectorEngine.parents(
          target,
        ...
        for (
          const item of SelectorEngine.prev(listGroup, SELECTOR_LINK_ITEMS)
        ) {
